...
public class ProjectileT extends VisEventAdapter
{
	...
	public void CalculateNextParabolas()
	{
		...
		{
			Parabola previousPar = bounces.get(i-1);
			double lt = previousPar.land_time;
			bounces.get(i).starttime = bounces.get(i-1).starttime + lt;
			//bounces.get(i).starttime = lt;

			double[] newx = new double[6];
			newx[0] = previousPar.parabola[0] + bounces.get(i-1).parabola[1]*lt;
			//newx[0] = bounces.get(i-1).parabola[0];
			newx[1] = previousPar.parabola[1];
			newx[2] = previousPar.parabola[2] + bounces.get(i-1).parabola[3]*lt;
			//newx[2] = bounces.get(i-1).parabola[2];
			newx[3] = previousPar.parabola[3];
			newx[4] = ball_radius;
			//newx[5] = (bounces.get(i-1).parabola[5] - g*(lt - bounces.get(i-1).starttime))*(-1*bounce_factor);
			newx[5] = (previousPar.parabola[5] - g*lt)*(-1*bounce_factor);
	...
	{
			Parabola currentPar = bounces.get(bindex);
			double a = -0.5*g;
			double b = currentPar.parabola[5]; //z_vel
			double c = currentPar.parabola[4] - ball_radius; //z_not - ball_radius
		...
		*/
	        ...
	        cur_landing[0] = currentPar.parabola[1]*land_time + currentPar.parabola[0];
	        cur_landing[1] = currentPar.parabola[3]*land_time + currentPar.parabola[2];
			...
			currentPar.land_time = land_time;
			currentPar.pred_landing = cur_landing;
			currentPar.valid = true;						
